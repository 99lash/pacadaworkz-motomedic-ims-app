...
const POSCategoryPanel = ({
  ...
  return (
    <div className={`flex items-center ${isCompact ? 'gap-1.5' : 'gap-2'} overflow-x-auto pb-2 scrollbar-hide no-scrollbar`}>
      {/* All Items Button */}
      <button
        onClick={onClearCategories}
        className={`flex items-center ${isCompact ? 'gap-1.5 px-3 py-1.5' : 'gap-2 px-4 py-2'} rounded-lg border transition-all whitespace-nowrap shrink-0 ${
      ...
      >
        <span className={`${isCompact ? 'text-xs' : ''} font-medium`}>All</span>
        <Badge
          className={`px-1.5 py-0 rounded-md ${isCompact ? 'text-[10px]' : 'px-2 py-0.5'} ${
            selectedCategoryIds.length === 0
      ...
      {categories.map((category) => {
        ...
        return (
          ...
          >
            <span className={`${isCompact ? 'text-xs' : ''} font-medium`}>{category.name}</span>
            <Badge
              className={`px-1.5 py-0 rounded-md ${isCompact ? 'text-[10px]' : 'px-2 py-0.5'} ${
                isSelected
                  ? "bg-primary text-primary-foreground"
